...
class MongoInstancesRepository:
    ...
    async def create_instance(self, instance: MongoInstance):
        result = await self._instances_collection.insert_one(instance.model_dump(exclude={"id"}))
    ...
    async def update_instance(self, instance_id: str, update):
        ...
        await self._instances_collection.update_one({"_id": ObjectId(instance_id)}, {"$set": updates})
