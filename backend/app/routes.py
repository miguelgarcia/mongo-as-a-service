...
class Routes:
    def __init__(self, instances_crud):
        self._instances_crud = instances_crud
        ...
        router.put("/instances/{instance_id}", response_model=serialization.MongoInstanceOut)(self.update_instance)
    ...
    async def create_instance(self, data: serialization.MongoInstanceCreate):
        return await self._instances_crud.create_instance(data.name)

    async def list_instances(self):
        return await self._instances_crud.get_all_instances()

    async def get_instance(self, instance_id: str):
        instance = await self._instances_crud.get_instance(instance_id)
        if not instance:
            raise HTTPException(status_code=404, detail="Instance not found")
        return instance

    async def update_instance(self, instance_id: str, data: serialization.MongoInstanceCreate):
        return await self._instances_crud.update_instance(instance_id, data.name)

    async def delete_instance(self, instance_id: str):
        await self._instances_crud.delete_instance(instance_id)
